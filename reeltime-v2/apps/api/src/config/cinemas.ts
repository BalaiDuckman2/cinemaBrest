export interface CinemaConfig {
  name: string;
  allocineId: string;
  city: string;
  address: string;
  latitude: number;
  longitude: number;
}

export const CINEMAS: CinemaConfig[] = [
  {
    allocineId: 'P0153',
    name: 'Les Studios',
    address: '136 Rue Jean Jaurès, 29200 Brest',
    city: 'Brest',
    latitude: 48.3886,
    longitude: -4.4942,
  },
  {
    allocineId: 'P0151',
    name: 'CGR Brest Le Celtic',
    address: '38 Rue de Glasgow, 29200 Brest',
    city: 'Brest',
    latitude: 48.3897,
    longitude: -4.4864,
  },
  {
    allocineId: 'P0417',
    name: 'Multiplexe Liberté',
    address: '2 Rue de la Porte, 29200 Brest',
    city: 'Brest',
    latitude: 48.3904,
    longitude: -4.4861,
  },
  {
    allocineId: 'W2920',
    name: 'Pathé Capucins',
    address: 'Centre Commercial Les Capucins, 29200 Brest',
    city: 'Brest',
    latitude: 48.3838,
    longitude: -4.4977,
  },
  {
    allocineId: 'G02PD',
    name: 'Ciné Galaxy',
    address: 'Zone de Mescoat, 29800 Landerneau',
    city: 'Landerneau',
    latitude: 48.4474,
    longitude: -4.2544,
  },
  {
    allocineId: 'P1883',
    name: 'La Salamandre',
    address: '39 ter Quai du Léon, 29600 Morlaix',
    city: 'Morlaix',
    latitude: 48.5831,
    longitude: -3.8344,
  },
  {
    allocineId: 'G0FOX',
    name: 'Cinéville Morlaix',
    address: '2 rue Karine Ruby, 29600 Plourin-lès-Morlaix',
    city: 'Morlaix',
    latitude: 48.5348,
    longitude: -3.7905,
  },
  {
    allocineId: 'P0299',
    name: 'Cinéville Quimper',
    address: '67 rue de la Providence, 29000 Quimper',
    city: 'Quimper',
    latitude: 48.0006,
    longitude: -4.1100,
  },
  {
    allocineId: 'P0633',
    name: 'Katorza',
    address: '38 boulevard Dupleix, 29000 Quimper',
    city: 'Quimper',
    latitude: 47.9946,
    longitude: -4.1021,
  },
  {
    allocineId: 'W2900',
    name: 'Quai Dupleix',
    address: '38 boulevard Dupleix, 29000 Quimper',
    city: 'Quimper',
    latitude: 47.9946,
    longitude: -4.1021,
  },
  // Troyes : hors zone bretonne. Le web masque ces salles tant que la ville
  // n'est pas choisie explicitement (voir web `utils/cinemaFilter.ts`).
  {
    allocineId: 'P0983',
    name: 'CGR Troyes',
    address: '13 rue des Bas-Trévois, 10000 Troyes',
    city: 'Troyes',
    latitude: 48.2924,
    longitude: 4.0799,
  },
  {
    // Pont-Sainte-Marie jouxte Troyes : rattaché à la ville comme Cinéville
    // l'est à Morlaix depuis Plourin.
    allocineId: 'W1015',
    name: 'Utopia Pont-Sainte-Marie',
    address: '11 rue du Moulinet, 10150 Pont-Sainte-Marie',
    city: 'Troyes',
    latitude: 48.3182,
    longitude: 4.1083,
  },
];

export function getCinemaByAllocineId(id: string): CinemaConfig | undefined {
  return CINEMAS.find((c) => c.allocineId === id);
}

/**
 * Build a film-specific booking URL.
 * - Les Studios: direct film page (uses AlloCiné film ID)
 * - All others: AlloCiné film page (universal, always film-specific)
 */
export function buildBookingUrl(cinemaAllocineId: string, filmAllocineId: number): string {
  if (cinemaAllocineId === 'P0153') {
    return `https://www.cine-studios.fr/film/${filmAllocineId}/`;
  }
  return `https://www.allocine.fr/film/fichefilm_gen_cfilm=${filmAllocineId}.html`;
}
